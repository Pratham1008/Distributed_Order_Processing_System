...
export default function HealthStatus() {
    ...
    useEffect(() => {
        const checkHealth = async () => {
            const baseUrl = getBaseUrl(false); // client-side URL
    ...
    const degradedServices = Object.entries(statuses).filter(([_, status]) => status === 'DOWN').map(([name]) => name);

    return (
        <div className="relative group hidden lg:flex items-center gap-4 bg-surface-container-highest px-4 py-1.5 rounded-full border border-surface-dim mr-2 cursor-pointer">
                    ...
                    {Object.entries(statuses).map(([name, status]) => (
                        <div key={name} className="flex items-center justify-between px-4 py-2 hover:bg-surface-container-low transition-colors">
                            ...
                            <div className="flex items-center gap-1.5">
                                {status === 'LOADING' && <span className="material-symbols-outlined animate-spin text-[16px] text-on-surface-variant">progress_activity</span>}
                                {status === 'UP' && <span className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_4px_rgba(34,197,94,0.5)]"></span>}
                                {status === 'DOWN' && <span className="w-2 h-2 rounded-full bg-red-500 shadow-[0_0_4px_rgba(239,68,68,0.5)]"></span>}
